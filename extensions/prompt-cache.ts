...
function getSystemText(payload: Record<string, unknown>): string | undefined {
  ...
  if (Array.isArray(payload.messages)) {
    const sys = payload.messages.find(
      (m) => typeof m === 'object' && m !== null && (m as Record<string, unknown>).role === 'system' || (m as Record<string, unknown>).role === 'developer'
    );
...
export function computePromptCacheKey(payload: Record<string, unknown>): string | undefined {
