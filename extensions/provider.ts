...
const findModel = (
	...
	if (m2) {
		findModelCache.set(cacheKey, m2);
		return m2;
	}
	...
	Object.entries(config.models).map(([name, cfg]) => {
		...
		const thinkingLevelMap =
			thinkingLevelSet.size > 0
				? (Object.fromEntries(
						[...thinkingLevelSet].map((l) => [l, l]),
					) as Partial<Record<ModelThinkingLevel, string | null>>)
				: undefined;
	...
	while (true) {
		...
		if (signal) {
			...
			if (signal.aborted) {
				...
				result = { done: true, value: undefined } as unknown as Awaited<
		...
		if (event.type === "error" && !contentReceived) {
			...
			const errMsg =
				...
				`Model ${ref} failed before sending content`;
...
const routeStream = (
	...
	(async () => {
		...
		if (!registry) {
			stream.push({
				...
				error: createErrorMessage(
					model,
					"Router not initialized — model registry unavailable",
				),
			});
			stream.end();
			return;
			...
			if (targetModel.contextWindow) {
				try {
					...
					if (totalChars > maxChars) {
						ctx = {
							...ctx,
							messages: truncateMessages(ctx.messages, maxChars),
						};
					}
				} catch {
		...
		if (candidates.every((ref) => isRateLimited(ref))) {
			...
			errorMsg =
				`Request dibatalkan setelah ${elapsed}s: pi menghentikan permintaan. ` +
